...
function App() {
  ...
  const cards = board?.leads.filter(card =>
  ...
  return <div className="kanban-workspace"><nav className="section-tabs" aria-label="Разделы заказов"><a href="/kanban/" aria-current="page">Доска</a><a href="/operator/">Список</a><a href="/calls/">Входящие звонки</a></nav><div className="page-title"><div><p className="eyebrow">КЛИЕНТЫ И ЗАКАЗЫ</p><h1>Заказы</h1><p className="subtitle">От обращения до оплаты — вся работа на одной доске.</p></div>{board?.can_add && <a className="button primary" href="/workspace/lead/new/">+ Новый лид</a>}</div>
